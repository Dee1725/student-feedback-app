...
const FeedbackForm = ({ onSubmitFeedback }) => {
  ...
  const validateForm = () => {
    ...
    else if (formData.studentName.length < 2) newErrors.studentName = 'Name must be at least 2 characters';
    if (!formData.courseCode.trim()) newErrors.courseCode = 'Course code is required';
    else if (formData.courseCode.length < 3) newErrors.courseCode = 'Course code must be at least 3 characters';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({});
      if (onSubmitFeedback) onSubmitFeedback(response.data); // send new feedback to parent
    } catch (error) {
